...
class GA_agg:
    ...
    def __init__(self,time_deadline,problem_path,init="cluster",mode="agglomerative",**kwargs): #the default values for these parameters should be the best values found in the experimental optimization of the algorithm.
    ...
    def run(self,event,individuals=300, crossovers= 1, max_iter=10000000, objective_value=0.2,proba_selection = [0.5,0.5]):
...
class GA2_agg:
    ...
    def __init__(self,pop,time_deadline,problem_path,init="cluster",mode="agglomerative",**kwargs): #the default values for these parameters should be the best values found in the experimental optimization of the algorithm.
        self.problem_path = problem_path
        self.best_solution = None 
        self.time_deadline = time_deadline 
        self.best_fitness = None
    ...
    def run(self,event,individuals=300, crossovers= 1, max_iter=10000000, objective_value=0.2,proba_selection = [0.5,0.5]):
        ...
        while (self.best_fitness is not None and self.best_fitness < objective_value and n_iter < max_iter) or  self.best_fitness is None: #Termination condition
            ...
            for i in range(crossovers):  #Iterations specified in the configuration (10 by default)
                ...
                population, fitness = self.replace_cmin(fitness, child3)
...
if __name__ == '__main__':
    dicc = {}
    instances = ['instance1.txt', 'instance02.txt', 'instance03.txt', 'instance04.txt', 'instance05.txt', 'instance06.txt', 'instance07.txt', 'instance08.txt']
    tiempo = 30
    cluster_agg = []
    cluster_agg2 = []
    normal = []
    for m in instances:
        ...
        for i in range(10):
            ...
            a = GA_agg(time_deadline=tiempo,problem_path=m,init="cluster",mode="agglomerative")
            ...
            a2 = GA2_agg(time_deadline=tiempo,pop=a.ini_pop, problem_path=m,init="cluster",mode="agglomerative")

            try:
                task_thread = threading.Thread(target= a2.run_with_timeout,args=(event,))
                task_thread.start()
                task_thread.join(a.time_deadline)
                event.set()
  
                sol2 = a2.best_fitness
                cluster_agg2.append(sol2)
            ...
            a3 = GA_agg(time_deadline=tiempo,problem_path=m, init='normal')
            try:
                task_thread = threading.Thread(target= a3.run_with_timeout,args=(event,))
                task_thread.start()
                task_thread.join(a.time_deadline)
                event.set()

                sol3 = a3.best_fitness
                normal.append(sol3)
        ...
        dicc[m][1] = np.mean(sol)
        dicc[m][2] = np.mean(sol2)
        dicc[m][3] = np.mean(sol3)
